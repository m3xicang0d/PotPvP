...
class Redis : Closeable {
    ...
    fun load(localCredentials: RedisCredentials, backboneCredentials: RedisCredentials) {
        try {
            ...
            localJedisPool = JedisPool(
                JedisPoolConfig(),
                localCredentials.host,
                localCredentials.port,
                TIMEOUT,
                password,
                localCredentials.dbId
            )
        } catch (e: Exception) {
            PotPvPSI.instance.logger.warning("Couldn't connect to a Redis instance at ${localCredentials.host}:${localCredentials.port}")
            e.printStackTrace()
        }
        ...
        try {
            ...
            backboneJedisPool = JedisPool(
                JedisPoolConfig(),
                backboneCredentials.host,
                backboneCredentials.port,
                TIMEOUT,
                password,
                backboneCredentials.dbId
            )
        } catch (e: Exception) {
            PotPvPSI.instance.logger.warning("Couldn't connect to a Backbone Redis instance at ${backboneCredentials.host}:${backboneCredentials.port}")
            e.printStackTrace()
        }
    }
