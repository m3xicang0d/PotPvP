    ...
    public void onButtonPress(InventoryClickEvent event) {
        ...
        if (openMenu != null) {
            ...
            if (openMenu.getButtons().containsKey(event.getSlot())) {
                ...
                player.setMetadata("CLICKED_BUTTON",new FixedMetadataValue(PotPvPSI.getInstance(),player.getUniqueId().toString()));
                ...
                if (event.isCancelled()) {
                    PotPvPSI.getInstance().getServer().getScheduler().runTaskLater(PotPvPSI.getInstance(), player::updateInventory, 1L);
                }

                player.removeMetadata("CLICKED_BUTTON", PotPvPSI.getInstance());
